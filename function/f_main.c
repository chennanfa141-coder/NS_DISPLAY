...
LOCALD void UpdateTestDisplayBuffer(void);
...
// 初始化功能码
    InitFuncCode(); //上电显示，屏蔽一部分只保留err显示作为显示测试

// 某些初始化应该在功能码正常读取之后进行

// 参数初始化
    FuncVariableInit(); // 菜单初始化
}
...
//    Uint16 initTicker = 0;
    ...
    baseTime = GetTime(); 
						#if 0
					GPIO_clearPin(BOARD_LED2_PIN); //led test
	#else
...
#endif
 #if 0
        if (errorEeprom)   // EEPROM故障 //orig   if (errorEeprom)  
    {
