...
#define    DSP_CLOCK200			200		                     //200MHz时钟 
...
#endif
#ifdef      DSP_CLOCK200
    #define     DSP_CLOCK           200     // CPU主频真实跑到 200MHz
    #define     PWM_CLK_DIV         0
    #define     PWM_CLOCK           200     // PWM硬件模块没有分频，也是 200MHz
    // 原来 60MHz 时：30 / 60MHz = 0.5微秒 (500ns)
    // 现在 200MHz 时，要得到相同的 0.5微秒：0.5us * 200MHz = 100
    #define     DBTIME_1140V        100     // 1140V 死区时间 (真实物理时间仍为 0.5us)

    // 原来 60MHz 时：8 / 60MHz = 0.133微秒 (133ns)
    // 现在 200MHz 时，要得到相同的 133ns：0.133us * 200MHz ≈ 27
    #define     DCTIME_1140V        27      // 1140V 死区补偿
#endif

#define     PWM_CLOCK_HALF      (PWM_CLOCK/2)               //PWM时钟除以2
#define     C_TIME_05MS         ((DSP_CLOCK/8) * 500L)      // 0.5ms: 25 * 500 = 12500 
#define     C_TIME_20MS         ((DSP_CLOCK/8) * 2000L)     //  20ms: 25 * 2000 = 50000 
#define     C_TIME_045MS        ((DSP_CLOCK/8) * 450L)      // 0.45ms: 25 * 450 = 11250
